...
## 🌟 Key Features
- **Contextual Correction**: Uses an N-gram frequency model trained on the `big.txt` corpus (Project Gutenberg) to suggest words that fit the sentence flow.
- **Intelligent Spelling**: Implements the **Levenshtein Distance** algorithm to find the closest matches for typos.
- **Grammar Insights**: Utilizes **NLTK Part-of-Speech (POS) tagging** to detect structural issues and suggest improvements.
- **Modern UI**: A sleek, user-friendly interface built with `tkinter` featuring a "Catppuccin-inspired" dark theme.
- **Real-time Feedback**: Highlights errors in red and warnings in orange within a side-by-side analysis view.
...
📂 Project Structure
spell_grammar_corrector_v2.py: The main Python script containing the NLP logic and the Tkinter GUI.

big.txt: The reference dataset used for training the frequency model and N-gram probabilities.
